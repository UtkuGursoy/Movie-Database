...
<div class="container my-5">
        <h2>MovieBox</h2>
        <a class="btn btn-primary" href="insert.php" role="button">Add Director</a>
        <br>
        <table class="table">
            <thead>
                <tr>
            ...
            <?php 
                    ...
                    if(isset($_POST['save_name']))
                    {
                        if(!empty($_POST['name'])){
                            $name = $_POST['name'];
                            $sql_statement = "SELECT * FROM directors_table WHERE director_name = '$name'" ; 

                            $result = mysqli_query($connection, $sql_statement); // Executing query
                            
                            if(mysqli_num_rows($result) == 0){
                                echo "Database does not contain such a director";
                            }
                            else{
